( function( $, _, document, window, rwmb, i18n ) {
	...
	group.toggle.initTitle = function ( $container ) {
		$container.find( '.rwmb-group-collapsible' ).each(
			function () {
				...
				$this.children( '.rwmb-input' ).each(
					function () {
						var $input = $( this );

						// Update group title.
						$input.children( '.rwmb-group-clone' ).each(
	...
	group.toggle.initState = function () {
		$( '.rwmb-group-collapsible.rwmb-group-collapsed' ).each(
			function () {
				var $this = $( this );
				if ( ! $this.hasClass( 'rwmb-group-non-cloneable' ) ) {
	...
	group.clone.processGroup = function ( event, index ) {
		...
		// Update [group index] for inputs
		$group.find( rwmb.inputSelectors ).each(
			function () {
