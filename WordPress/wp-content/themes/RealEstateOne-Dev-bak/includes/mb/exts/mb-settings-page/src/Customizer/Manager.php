...
class Manager {
	...
	public function init() {
		...
		$settings_sections = array_filter(
			$meta_boxes,
			function( $meta_box ) {
				return $meta_box->settings_pages && Factory::get( $meta_box->settings_pages, 'customizer' );
			}
		);
		array_walk( $settings_sections, array( $this, 'register_settings_section' ) );

		// Meta box that doesn't have a settings page.
		$normal_sections = array_filter(
			$meta_boxes,
			function( $meta_box ) {
				return isset( $meta_box->meta_box['panel'] );
			}
		);
		array_walk( $normal_sections, array( $this, 'register_normal_section' ) );
	}
