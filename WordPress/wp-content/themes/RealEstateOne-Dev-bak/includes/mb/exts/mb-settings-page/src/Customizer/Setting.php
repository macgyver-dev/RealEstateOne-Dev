...
class Setting {
	...
	public function register( $wp_customize ) {
		...
		// Save the setting.
		add_action( 'customize_update_meta_box', array( $this, 'save' ), 10, 2 );

		// Filter the option for previewing.
		add_action( "customize_preview_{$this->id}", array( $this, 'preview' ) );
	}
	...
	public function preview() {
		add_filter( "option_{$this->meta_box->object_id}", array( $this, 'preview_filter' ) );
		add_filter( "default_option_{$this->meta_box->object_id}", array( $this, 'preview_filter' ) );
	}
	...
	public function preview_filter( $original ) {
		...
		$original = empty( $original ) ? array() : $original;
